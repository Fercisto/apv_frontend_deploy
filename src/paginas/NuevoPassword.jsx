...
const NuevoPassword = () => {
  ...
  const handleSubmit = async e => {
    ...
    if(password.length < 6) {
      setAlerta({msg: 'El Password debe de ser mínimo 6 caracteres', error: true});
      return;
    }
        ...
        {tokenValido && (
          <>
            <form className="mt-5 space-y-5" onSubmit={handleSubmit}>
              <div>
                <label className="uppercase tex-gray-600 block text-xl font-bold" htmlFor="password">Nuevo Password</label>
                <input 
                  type="password" 
                  id="password" 
                ...
                <input type="submit" value="Guardar Nuevo Password" className="bg-indigo-700 w-full md:w-auto py-3 px-10 rounded-xl text-white uppercase font-bold hover:cursor-pointer hover:bg-indigo-800 transition-colors" />
